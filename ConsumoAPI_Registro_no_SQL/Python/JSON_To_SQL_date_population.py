...
def cria_tabela(response, conn, nome_tabela):
    ...
    if cursor.fetchone()[0] == 0: #se não existir ==0 /se existir == 1
        ...
        columns = ','.join([f'[{campo}] VARCHAR(MAX)' for campo in campos]) # Faz uma query com cada valor do campo, sendo um array pegando todos os dados do campo repitindo o tamanho 
...
def alter_table_column_size(conn, table_name, column_name, new_size):
    ...
    alter_query = "ALTER TABLE BdVexSoft ALTER COLUMN json_obj VARCHAR(MAX)"
    cursor.execute(alter_query)
    conn.commit()
...
data_inicial = datetime(2024, 5, 15)  # Defina a data inicial aqui
...
while data_final < data_atual:
    ...
    if response.status_code == 200:
        cria_tabela(response, conn, nome_tabela='BdVexSoft') #envia a requisicao, a conexao, #nome da tabela
        insere_valores(response, conn, table_name='BdVexSoft')
        alter_table_column_size(conn, 'BdVexSoft', 'fotos', 'MAX')
    else:
